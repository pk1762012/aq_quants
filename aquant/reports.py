...
def get_metrics_data(
    ...
    """
    Calculate financial metrics and return as a structured dictionary

    Parameters similar to original metrics() function but focused on data return
    Returns:
    dict: Organized metrics data with categories as keys
    """
    if match_dates:
        returns = returns.dropna()
    returns.index = returns.index.tz_localize(None)
    ...
    metrics_data['general'] = {
        'start_date': df.index[0].strftime('%Y-%m-%d'),
        'end_date': df.index[-1].strftime('%Y-%m-%d'),
        'risk_free_rate': rf,
        'time_in_market': _stats.exposure(df, prepare_returns=False)
    }

    # Calculate core ratios
    metrics_data['ratios'] = {
        'sharpe': _stats.sharpe(df, rf, win_year, False),
        'sortino': _stats.sortino(df, rf, win_year, False),
        'omega': _stats.omega(df['returns'], rf, 0.0, win_year),
        'gain_to_pain': _stats.gain_to_pain_ratio(df, rf),
        'profit_factor': _stats.profit_factor(df, prepare_returns=False)
    }

    # Calculate risk metrics
    metrics_data['risk'] = {
        'volatility': _stats.volatility(df, win_year, False, prepare_returns=False),
        'var': _stats.var(df, prepare_returns=False),
        'cvar': _stats.cvar(df, prepare_returns=False),
        'ulcer_index': _stats.ulcer_index(df)
    }

    # Calculate returns for different periods
    today = df.index[-1]
    metrics_data['returns'] = {
        'total': _stats.comp(df) if compounded else df.sum(),
        'cagr': _stats.cagr(df, rf, compounded),
        'mtd': _stats.comp(df[df.index >= pd.Timestamp(today.year, today.month, 1)]),
        'ytd': _stats.comp(df[df.index >= pd.Timestamp(today.year, 1, 1)]),
        '1y': _stats.comp(df[df.index >= today - pd.DateOffset(years=1)])
    }

    # Calculate drawdown metrics
    dd = _calc_dd(df, display=False)
    metrics_data['drawdown'] = {
        'max_drawdown': dd.loc['Max Drawdown %'].iloc[0],
        'avg_drawdown': dd.loc['Avg. Drawdown %'].iloc[0],
        'longest_dd_days': dd.loc['Longest DD Days'].iloc[0]
    }

    # Calculate timing metrics
    metrics_data['timing'] = {
        'win_rate': _stats.win_rate(df, prepare_returns=False),
        'best_day': _stats.best(df, prepare_returns=False),
        'worst_day': _stats.worst(df, prepare_returns=False)
    }

    if mode.lower() == "full":
        # Add additional metrics for full mode
        metrics_data['ratios'].update({
            'calmar': _stats.calmar(df, prepare_returns=False),
            'smart_sharpe': _stats.smart_sharpe(df, rf, win_year, False),
            'smart_sortino': _stats.smart_sortino(df, rf, win_year, False)
        })

        metrics_data['risk'].update({
            'skew': _stats.skew(df, prepare_returns=False),
            'kurtosis': _stats.kurtosis(df, prepare_returns=False),
            'risk_of_ruin': _stats.risk_of_ruin(df, prepare_returns=False)
        })
